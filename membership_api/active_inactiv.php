   <?php
    ...
    {
        $str = $db->query("UPDATE `ss_setup_user_member_type` SET `is_active`='1' where user_member_type_id =$productid");
        $success = 'On';
        echo json_encode(array('response_result' => $response_result, '$success' => $success));
    } else if ($mode == 'false') {
        $str = $db->query("UPDATE `ss_setup_user_member_type` SET `is_active`='0' where user_member_type_id =$productid");
