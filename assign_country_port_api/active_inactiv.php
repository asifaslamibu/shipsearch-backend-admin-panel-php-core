   <?php
    ...
    {
        $str = $db->query("UPDATE `ss_setup_region_country_port` SET `is_active`='1' where id=$productid");
        $success = 'On';
        echo json_encode(array('response_result' => $response_result, '$success' => $success));
    } else if ($mode == 'false') {
        $str = $db->query("UPDATE `ss_setup_region_country_port` SET `is_active`='0' where id=$productid");
